...
class SVHN(object):
    ...
    def __init__(self, data_dir):
        ...
        # getting images to numpy temp_x is the input images for self.x
        ...
        # print self.h_pool3.shape
            ...
            # fourth conv layer
        self.W_conv4 = self.weight_variable([5,5,128,150])
        self.b_conv4 = self.bias_variable([150])
        ...
        # print self.h_pool4.shape
            ...
            # fifth conv layer
        self.W_conv5 = self.weight_variable([3,3,150,160])
        self.b_conv5 = self.bias_variable([160])
        ...
        self.W_fc1 = self.weight_variable([2*2*160, 1024])
        self.b_fc1 = self.bias_variable([1024])
        self.h_pool2_flat = tf.reshape(self.h_pool5, [-1, 2*2*160])
        ...
        # print self.cross_entropy1.shape
        ...
        # print self.final_entropy.shape

        self.train_step = tf.train.AdamOptimizer(1e-4).minimize(self.final_entropy)
        # determining trainstep
        # self.train_step_len = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy_len)
        # self.train_step_1 = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy1)
        # self.train_step_2 = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy2)
        # self.train_step_3 = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy3)
        # self.train_step_4 = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy4)
        # self.train_step_5 = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy5)

        # determining correctprediction
    ...
    def train(self):
        ...
        # print self.sess.run(self.W_conv1)
        # self.sess.run(tf.initialize_all_variables())

        # print self.y_digit_length
        for i in range (1,3):
            self.temp_x = self.getImages()
            self.temp_y_digit_length,self.temp_y_digit1,self.temp_y_digit2,self.temp_y_digit3,self.temp_y_digit4,self.temp_y_digit5 = self.getDigits()
            # print 'self.temp_y_digit_length '+str(self.temp_y_digit_length)
            # print 'self.temp_y_digit1'+str(self.temp_y_digit1)
            # print 'self.temp_y_digit2'+str(self.temp_y_digit2)
            # print 'self.temp_y_digit3'+str(self.temp_y_digit3)
            # print 'self.temp_y_digit4'+str(self.temp_y_digit4)
            # print 'self.temp_y_digit5'+str(self.temp_y_digit5)
            a,b,c,d,e,f,g = self.sess.run([self.train_step, self.final_entropy,self.cross_entropy1,self.cross_entropy2,
            ...
            print str(i) + '  final entropy  '+str(b)
                # print 'cross_entropy1'+str(c)
                # print 'cross_entropy2'+str(d)
                # print 'cross_entropy3'+str(e)
                # print 'cross_entropy4'+str(f)
                # print 'cross_entropy_len'+str(g)
        img = cv2.imread(self.data_directory+'cr/32607.png')
        arr = self.get_sequence(img)
        print arr

        img = cv2.imread(self.data_directory+'cr/32608.png')
        arr = self.get_sequence(img)
        print arr

        img = cv2.imread(self.data_directory+'cr/32609.png')
        arr = self.get_sequence(img)
        print arr

        img = cv2.imread(self.data_directory+'cr/32610.png')
        arr = self.get_sequence(img)
        print arr

        img = cv2.imread(self.data_directory+'cr/32611.png')
        arr = self.get_sequence(img)
        print arr

        # self.save_model(name='khushal')
        # self.save_model(self.W_conv1)
...
if __name__ == "__main__":
    ...
    # obj = SVHN('data/')
    # obj.train()
    obj = SVHN.load_model(name = 'data/')
    img = cv2.imread('data/cr/29001.png')

    a = obj.get_sequence(img)
    print a
    # obj.preprocess()
    # x=0
